...
def test_is_deterministic(fitted_logit):
    context = fitted_logit
    kwargs = {
        "y": context["y_test"], "X_test": context["X_test"],
        "X_train": context["X_train"], "model": context["model"],
        "yhat": context["yhat"], "variables": ["income", "dti"],
        "protected": "gender", "random_state": 7,
    }
...
def test_partial_specification_warns_instead_of_failing(fitted_logit):
    ...
    minimal = rgbox_report(
        y=context["y_test"], X_test=context["X_test"],
        model=context["model"], yhat=context["yhat"],
    )
...
def test_explainability_skipped_without_train_frame(fitted_logit):
    context = fitted_logit
    result = rgbox_report(
        y=context["y_test"], X_test=context["X_test"], model=context["model"],
        yhat=context["yhat"], variables=["income"],
    )
...
def test_one_hot_protected_attribute_runs_end_to_end(fitted_logit):
    ...
    result = rgbox_report(
        y=context["y_test"], X_test=context["X_test"], X_train=context["X_train"],
        model=context["model"], yhat=context["yhat"], protected=dummies,
        random_state=0,
    )
...
def test_tiny_protected_group_is_flagged_in_warnings(fitted_logit):
    ...
    result = rgbox_report(
        y=context["y_test"], X_test=context["X_test"], X_train=context["X_train"],
        model=context["model"], yhat=context["yhat"], protected="gender",
